...
import F09_binaryTree.T17_lowestAncestor;
...
public class C17 {
    ...
    public static class Info {
        ...
        public Info(Node resNode, boolean isA, boolean isB) {
    ...
    public static Node job(Node head, Node a, Node b) {
        ...
        Info process = process(head, a, b);
        return process.resNode;
    }
    ...
    public static Info process(Node head, Node a, Node b) {
        ...
        boolean isA = (head == a) || left.isA || right.isA;
        boolean isB = (head == b) || left.isB || right.isB;
        Node resNode = null;
        if (left.resNode != null){
            resNode = left.resNode;
        }else if (right.resNode != null){
            resNode = right.resNode;
        }else {
            if (isA && isB){
                resNode = head;
            }
        }
        return new Info(resNode,isA, isB);
    }
